            ...
            foreach (var item in ObjectNameConfiguration.Split(';')) {
                ...
                switch (kv[0]) {
                    ...
                    case "name":
                        ...
                        break;
                    case "size":
                        stats.Add(new KeyValuePair<StatsType, object>(StatsType.Size,
                            Math.Min(500, Utils.GetInt(kv[1]))));
                        break;
                    //case "eff":
    ...
    public int LoadFromMapData(MapData data, int idBase) {
        ...
        for (var x = 0; x < Width; x++) {
            ...
            if (tile.ObjectType != 0 &&
                (tileData.ObjectDesc == null || !tileData.ObjectDesc.Static || tileData.ObjectDesc.Enemy)) {
                entities.Add(new Tuple<IntPoint, ushort, string>(new IntPoint(x, y), tile.ObjectType,
                    tile.ObjectNameConfiguration));
                if (tileData.ObjectDesc == null || !(tileData.ObjectDesc.Enemy && tileData.ObjectDesc.Static))
                    tile.ObjectType = 0;
            }

            if (tile.ObjectType != 0 &&
                (tileData.ObjectDesc == null || !(tileData.ObjectDesc.Enemy && tileData.ObjectDesc.Static))) {
                enCount++;
