...
internal class Oryx {
    ...
    private readonly List<Tuple<string, ISetPiece>> _events = new() {
    ...
    private readonly List<Tuple<string, ISetPiece>> _rareEvents = new();

    private readonly RealmOfTheMadGod _world;
    private long _prevTick;
    private int _tenSecondTick;
    public bool Closing;
    private RealmTime dummy_rt = new();
    ...
    public void OnEnemyKilled(Enemy enemy, Player killer) {
        ...
        foreach (var player in _world.Players) player.Value.HandleQuest(dummy_rt, true);
    }
