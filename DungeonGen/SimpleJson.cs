...
namespace Json
{
		...
		static class SimpleJson
	{
				...
				if (c == '\\')
				{
					...
					else if (c == 'u')
					{
						...
						if (remainingLength >= 4)
						{
							...
							if (
								!(success =
									UInt32.TryParse(new string(json, index, 4), NumberStyles.HexNumber, CultureInfo.InvariantCulture, out codePoint)))
								return "";
							...
							if (0xD800 <= codePoint && codePoint <= 0xDBFF) // if high surrogate
							{
								...
								if (remainingLength >= 6)
								{
									...
									if (new string(json, index, 2) == "\\u" &&
									    UInt32.TryParse(new string(json, index + 2, 4), NumberStyles.HexNumber, CultureInfo.InvariantCulture,
										    out lowCodePoint))
									{
		...
		private static object ParseNumber(char[] json, ref int index, ref bool success)
		{
			...
			{
				long number;
				success = long.TryParse(new string(json, index, charLength), NumberStyles.Any, CultureInfo.InvariantCulture,
					out number);
				returnNumber = number;
			}
			index = lastIndex + 1;
 ...
 DataContractJsonSerializerStrategy
		...
		class PocoJsonSerializerStrategy : IJsonSerializerStrategy
	{
		...
		{
			ConstructorCache =
				new ReflectionUtils.ThreadSafeDictionary<Type, ReflectionUtils.ConstructorDelegate>(ContructorDelegateFactory);
			GetCache =
			...
			if (str != null)
			{
				if (str.Length != 0) // We know it can't be null now.
				{
					if (type == typeof(DateTime) ||
					    (ReflectionUtils.IsNullableType(type) && Nullable.GetUnderlyingType(type) == typeof(DateTime)))
					...
					if (type == typeof(DateTimeOffset) ||
						...
						return DateTimeOffset.ParseExact(str, Iso8601Format, CultureInfo.InvariantCulture,
							DateTimeStyles.AssumeUniversal | DateTimeStyles.AdjustToUniversal);
					if (type == typeof(Guid) ||
					    (ReflectionUtils.IsNullableType(type) && Nullable.GetUnderlyingType(type) == typeof(Guid)))
						return new Guid(str);
					if (type == typeof(Uri))
					{
				...
				else
				{
					...
					if (valueAsList != null)
					{
						         ...
						         ReflectionUtils.IsAssignableFrom(typeof(IList), type))
						{
							...
							list =
								(IList)
									(ConstructorCache[type] ?? ConstructorCache[typeof(List<>).MakeGenericType(innerType)])(jsonObject.Count);
							foreach (object o in jsonObject)
			...
			public static GetDelegate GetGetMethodByExpression(PropertyInfo propertyInfo)
			{
				...
				Func<object, object> compiled =
					Expression.Lambda<Func<object, object>>(
						Expression.TypeAs(Expression.Call(instanceCast, getMethodInfo), typeof(object)), instance).Compile();
