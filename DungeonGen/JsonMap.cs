...
public static class JsonMap {
    public static DungeonTile[,] Load(string json) {
        ...
        foreach (JsonObject tile in (JsonArray) map["dict"]) {
            ...
            tile.TryGetValue("ground", out var tileType);
            mapTile.TileType = new TileType(tileType == "Space" ? 0xfe : (uint) tileType.GetHashCode(),
                tileType.ToString());

            mapTile.Region = tile.ContainsKey("regions")
                ? (string) ((JsonObject) ((JsonArray) tile["regions"])[0])["id"]
                : null;
        ...
        var data = Zlib.Decompress(Convert.FromBase64String((string) map["data"]));
        var index = 0;
        for (var y = 0; y < h; y++)
        for (var x = 0; x < w; x++)
    ...
    public static string Save(DungeonTile[,] map) {
        ...
        for (var x = 0; x < w; x++) {
            var tile = map[x, y];
            if (!indexLookup.TryGetValue(tile, out var index)) {
