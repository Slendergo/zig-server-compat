...
namespace wServer.realm.worlds.parser
{
    public sealed class TileData
    {
        ...
        {
            TileDesc = Program.Resources.GameData.Tiles.GetValueOrDefault(rdr.ReadUInt16(), Program.Resources.GameData.Tiles[0xFF]);
    ...
    public sealed class MapData
    {
        ...
        {
            /*StartX*/ _ = rdr.ReadUInt16();
            /*StartY*/ _ = rdr.ReadUInt16();
    ...
    public sealed class MapParser
    {
        ...
        private static readonly Dictionary<string, MapData> Cache = new Dictionary<string, MapData>();
...
#if DEBUG
                if (data == null)
                    Console.WriteLine($"Unable to locate: {map} in MapParser cache");
                else
                    Console.WriteLine($"Cached: {map}");
#endif
                return data;
            }
        }
        ...
        private static MapData TryCacheMap(string map)
        {
            ...
            using var rdr = new BinaryReader(new ZlibStream(new MemoryStream(File.ReadAllBytes(path)), CompressionMode.Decompress));
        ...
        public static byte[] ConvertWmapRealmToMapData(byte[] data)
        {
            ...
            foreach (var tile in dict)
            {
                ...
                wtr.Write((ushort)tile.TileType);
                wtr.Write((ushort)tile.ObjectType);
